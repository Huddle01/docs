---
title: 'Create Room API'
---

import CreateRoom from '@components/ApiRef/CreateRoom';
import CreateRoomResponse from '@components/ApiRef/CreateRoomResponse';
import RequestStrip from '@components/ApiRequest/RequestStrip';

import {
  APIRequestCard,
  APIRequestContent,
  APIResponseCard,
} from '@components/common/ApiCard/ApiCard';
import { Button } from '@components/common/Button/Button';
import HighLight from '@components/common/HighLight';
import Paragraph from '@components/common/Paragraph';
import Pill from '@components/common/Pill/Pill';
import { Callout } from 'nextra-theme-docs';

# Create IFrame Room API

The Huddle01 API provides the ability to create rooms, join room tokens for hosting various communication experiences.
In order to use the API, you will need to obtain an API key by connecting to your wallet.

<Callout>
If you have a custom Huddle01 subdomain, use the [Create Room API](/apis/create-room/create-room) instead
</Callout>

## **Endpoint**

<div className="flex justify-between md:flex-row flex-col">
  <HighLight className="my-3 md:w-1/2 w-full">
    <RequestStrip
      method={'POST'}
      url={'https://api.huddle01.com/api/v1'}
      endpoint={'create-iframe-room'}
    />
  </HighLight>
  <Button
    variant="primary"
    url="/api-keys"
    text="Get your API Key"
    isRouting
    className=" my-2 w-auto px-8"
  />
</div>

<Paragraph className="mt-5">
  The Create Iframe Room API is to create a meeting room and obtain roomId, and meetingLink.
  Additionally, you can add custom functionalities such as `muteOnEntry`,
  `roomLock`, and `hostWallets`, among others, to enhance the functionality of
  your iframe meeting rooms.
</Paragraph>

<div className="grid md:grid-cols-2 gap-4 grid-cols-1 mt-10  md:h-[24.5rem]">
<div>
<div className="flex items-center gap-4">
<Pill >POST</Pill>
<div className="text-sm font-normal text-slate-50">/create-iframe-room</div>

</div>

<div className="mt-7 font-mono text-2xl font-bold text-[#f1f5f9]">
  Create Iframe Room Api
</div>

The API supports a **POST** request with the following required parameters:

- **`title`** - The title of the room.
- **`roomLocked`** - Boolean value to lock the room.

</div>

<APIRequestCard
    requestType = "POST"
    route="/create-iframe-room"
    
  >

<APIRequestContent value="axios">
      ```js
      const response = await axios.post(
        'https://api.huddle01.com/api/v1/create-iframe-room',
        {
          title: 'Huddle01-Test',
          roomLocked: true
        },
        {
          headers: {
            'Content-Type': 'application/json',
            'x-api-key': {{API_KEY}},
          },
        }
    );
      ```
    </APIRequestContent>

    
    <APIRequestContent value="fetch">
      ```js
     const  response = await fetch("https://api.huddle01.com/api/v1/create-iframe-room", {
     
    method: "POST",
    {
        title: "Huddle01-Test",
        roomLocked: true
    },
    headers: {
        "Content-type": "application/json",
        'x-api-key': {{API_KEY}},
    }
})
      ```
    </APIRequestContent>

    <APIRequestContent value="cUrl">
      ```bash
curl -X POST \
  'https://api.huddle01.com/api/v1/create-iframe-room' \
  --header 'Accept: */*' \
  --header 'x-api-key: {{API_KEY}}' \
  --header 'Content-Type: application/json' \
  --data-raw '{
    "title" : "Huddle01-Test",
    "roomLocked": true
}'
      ```
    </APIRequestContent>
  </APIRequestCard>

</div>

<div className="grid md:grid-cols-2 gap-4 grid-cols-1 mt-10">
<div className="text-start ">

<div className="font-mono text-2xl font-bold text-[#f1f5f9]">Response</div>

Upon success, the API will return a response with the following fields:

- **`message`** - The message returned from the server.
- **`roomId`** - The id of the room created.


In case of an error, the API will return an error message with a corresponding status code.

</div>

<APIResponseCard>
  <APIRequestContent value="Success">
```jsx
{
    "message": "Meeting Created Successfully",
    "data": {
        "roomId": "tyt-eyoq-zwq",
        "meetingLink": "https://app.huddle01.com/tyt-eyoq-zwq"
    }
}
````

</APIRequestContent>

<APIRequestContent value="Error">

```jsx
{
    "message": "Input validation failed",
    "code": "BAD_REQUEST",
    "issues": [
        {
            "code": "invalid_type",
            "expected": "string",
            "received": "number",
            "path": [
                "title"
            ],
            "message": "Expected string, received number"
        }
    ]
}
```

</APIRequestContent>
  </APIResponseCard>

</div>

<div className = 'flex-col item-center gap-3' >

### **Request**

<CreateRoom />



### **Response**

<CreateRoomResponse />

</div>

