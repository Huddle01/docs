...
<APIRequestCard
  ...
  >
    <APIRequestContent value="cUrl">
      ```bash
        curl --location --request GET 'https://api.huddle01.com/api/v1/rooms/meetings?roomId={roomId}&limit=3&cursor=2' \
    ...
    <APIRequestContent value="fetch">
      ```js
        const  response = await fetch("https://api.huddle01.com/api/v1/rooms/meetings?roomId={roomId}&limit=3&cursor=2", {
          method: "GET",
          headers: {
