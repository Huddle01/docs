---
title: 'Video' 
---

import OptionTable from '@components/common/Table/OptionTable';
import { Callout } from 'nextra-theme-docs';

## Video 

### Enable Video 

The `enableVideo` method enables the local microphone and starts producing the video stream with the label `video`.

<Callout type='info'>
  `enableVideo` method calls both `fetchStream` and `produce` method internally. If you want to fetch video Stream, you have to explicitly call `fetchStream` method. 
  Refer to [Media Devices](/docs/Javascript/methods/mediaDevices) for more information.
</Callout>

**Example**

```js
// Enable Video stream
await huddleClient.localPeer.enableVideo();
```

- ### Disable Video

The `disableVideo` method stops the local video stream and also stops producing the video stream with remote peers.

**Example**

```js
// Disable video stream
await huddleClient.localPeer.disableVideo();
```


### replaceVideoStream

The `replacevideoStream` method replaces the current video stream with the new stream.

**Parameters**
<OptionTable
    headers={['Params',"Type", "Description" , 'Return Type']}
  options={[
    [
      'mediastream',
      "MediaStream",
      "This function replaces the video stream",
      'Promise<void>'
    ],
  ]}
/>

**Example**


```jsx
// Usage
await huddleClient.localPeer.replaceVideoStream({MediaStream});
```


### changeVideoSource

The `changeVideoSource` changes the Video source to the given deviceId, sets the preferred cam device as the given deviceId

**Parameters**
<OptionTable
    headers={['Params',"Type","Description" , 'Return Type']}
  options={[
    [
      'deviceId',
      "string",
      "This function changes the video stream",
      'Promise<void>'
    ],
  ]}
/>

**Example**


```jsx
// Usage
await huddleClient.localPeer.changeVideoSource({deviceId});
```