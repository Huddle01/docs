...
To power your Vanilla JS App with audio/video communication using Huddle01.
 ...
 <Tabs items={['pnpm', 'npm', 'yarn']}>
              <Tab>
            ```bash 
      pnpm i @huddle01/web-core@alpha
    ```
            </Tab>
  <Tab>
      ```bash 
    npm i @huddle01/web-core@alpha
    ```
            </Tab>
  <Tab>
      ```bash 
    yarn add @huddle01/web-core@alpha
    ```
            </Tab>
              </Tabs>
...
We have created a [token simulator](https://github.com/Huddle01/huddle01-token-simulator) which you can use to generate room and access token.
...
export function handleVideoStream(element) {
  element.addEventListener('click', async () => {
    ...
    // This will fetch video stream and return object from which we can get stream
    const streamResponse = await client.localPeer.deviceHandler.fetchStream({
      mediaDeviceKind: 'cam',
    });

    // This will return stream from the object
    const stream = streamResponse.stream;

    // This will produce the stream to other participants
...
export function handleScreenStream(element) {
  element.addEventListener('click', async () => {
    ...
    // This will fetch screen share stream and return object from which we can get stream
...
Full code of this walkthrough can be found [here](https://github.com/Huddle01/alpha-esm-js-example). 
