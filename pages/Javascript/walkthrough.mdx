...
<script type="module">
  ...
  document.getElementById('enableCam').onclick = async () => {
    ...
    // Produce the stream
    await huddleClient.localPeer.produce({
      label: 'video',
      stream: stream,
    });
  ...
  document.getElementById('disableScreen').onclick = async () => {
     // Get reference to the video element
     const videoRef = document.getElementById('video');
