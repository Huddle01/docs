...
By default, it will our predefined layout. 
</Callout>
...
You can then call the `startLivestream` method to start livestreaming a meeting by passing the room IDg. 
It accepts an array of `rtmpUrls` in which `rtmpUrl` is consist of `STREAM_URL` and `STREAM_KEY`  i.e. `YOUR_STREAM_URL/YOUR_STREAM_KEY`. 
