...
<Tabs items={['pnpm', 'npm', 'yarn']}>
  <Tab>
    ```bash 
      pnpm i @huddle01/react react-native-get-random-values react-native-webrtc 
    ```
  </Tab>
  <Tab>
    ```bash 
    npm i @huddle01/react react-native-get-random-values react-native-webrtc 
    ```
  </Tab>
  <Tab>
    ```bash 
    yarn add @huddle01/react react-native-get-random-values react-native-webrtc 
    ```
  </Tab>
</Tabs>
...
```tsx filename="app.tsx" highlight={9,14,51-65}
  ...
  const App = () => {
    ...
    return (
      <View>{isInitialized ? 'Hello World!' : 'Please initialize'}
        {/* Webcam */} 
        <Button 
