...
<Callout>
  This guide assumes you have a working React application with the ability to create and join Huddle01 rooms.
</Callout>
...
For this guide, we have already created a basic video conferencing web app using Next.js, and we will be adding recording and livestreaming functionality to it. To build a video conferencing app from scratch, you can refer to the [React Walkthrough](/React/react-walkthrough) guide.

## App Initial State

We have used the Huddle01 [React SDK](/React) to build this app.
This is what it looks like once you join the room and enable your video from two different tabs. 
...
To record a meeting, you have to make use of the [Huddle01 Server SDK](/Server-SDK). This SDK exposes functions that allow you to start and stop a meeting recording. This is supposed to happen on the server side, so you will need to create an API route to handle the recording.
...
<Tabs items={['pnpm', 'npm', 'yarn']}>
              <Tab>
            ```bash 
      pnpm i @huddle01/server-sdk@alpha
    ```
            </Tab>
  <Tab>
      ```bash 
    npm i @huddle01/server-sdk@alpha
    ```
            </Tab>
  <Tab>
      ```bash 
